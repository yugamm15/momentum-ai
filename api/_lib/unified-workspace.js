...
async function loadV2Snapshot(supabase, options = {}) {
  ...
  const meetings = (meetingsRows || []).map((meeting) => {
    ...
    const summaryParagraph = buildDisplaySummaryParagraph({
      ...
      meetingLabel: rawMetadata.meetingLabel || meeting.source_meeting_label || meeting.ai_title,
    });
...
function isUsableMeetingTitle(value) {
  ...
  const genericTitles = new Set([
    ...
    'momentum captured the audio file but the transcript contained too little clear speech for a reliable summary',
    'info',
    'infoinfo',
  ]);
